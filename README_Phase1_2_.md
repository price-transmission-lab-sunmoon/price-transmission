...
ADF-KPSS 상충: 3건 (쇠고기 PPI, 땅콩 도매가, 오렌지 CPI) → 보수적으로 비정상 처리 후 차분하여 전부 I(1) 확정.

### 4.7 I(2) 시계열 상세

| 품목 | 컬럼 | 수준 ADF p | 수준 KPSS p | 차분 ADF p | 차분 KPSS p | 원인 추정 |
| --- | --- | --- | --- | --- | --- | --- |
| 땅콩 | wholesale_price | 0.0441 | 0.0100 | 0.1478 | 0.1000 | 관측치 85개로 ADF 검정력 부족 |
| 오렌지 | intl_price_krw | 0.9986 | 0.0122 | 0.1721 | 0.0194 | 2024년 급등 추세가 차분 후에도 잔존 |

Phase 3 대응 방침:
- 두 시계열 모두 ADF p값이 0.15~0.17 수준으로 "확실한 I(2)"라고 보기 어렵다
- 해당 구간에서 I(1)로 간주하고 공적분 검정을 진행하되, 결과에 주의 플래그를 부착한다
- 논문에서 연구의 한계로 명시한다
...
### 6.1 seasonal_adjusted/{cid}_sa.csv
