...
출력:
  ...
  - tests/shap/results/{YYYYMMDD_HHMM}_IF/shap_summary.csv
  - tests/shap/results/{YYYYMMDD_HHMM}_IF/run_meta.json

Explainer: shap.TreeExplainer (exact SHAP, 수 초/품목)

실행:
  python tests/shap/run_shap_if.py
  python tests/shap/run_shap_if.py --run-date 20260519_0945
  python tests/shap/run_shap_if.py --output-dir tests/shap/results/20260519_1910_IF  (재실행/이어하기)
"""
...
def run_shap_if(data_dir, ml_dir, output_base, run_date=None, output_dir_override=None):
    ...
    # 출력 디렉토리: --output-dir 지정 시 해당 경로, 아니면 새 타임스탬프
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SHAP — Isolation Forest")
    ...
    run_shap_if(DATA_DIR, ML_DIR, OUTPUT_BASE, run_date=args.run_date, output_dir_override=args.output_dir)